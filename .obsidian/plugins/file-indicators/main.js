...
// node_modules/.pnpm/binary-insert@1.2.1/node_modules/binary-insert/dist/index.js
var require_dist = __commonJS({
  "node_modules/.pnpm/binary-insert@1.2.1/node_modules/binary-insert/dist/index.js"(exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.binaryInsert = void 0;
    function binaryInsert2(array, insertValue, comparator) {
      ...
      var left2 = 0, right2 = array.length;
      var leftLast = 0, rightLast = right2;
      while (left2 < right2) {
        var inPos = Math.floor((right2 + left2) / 2);
        ...
        if (leftLast === left2 && rightLast === right2) {
          break;
        }
        leftLast = left2;
        rightLast = right2;
      }
      array.splice(right2, 0, insertValue);
      return array;
    }
    exports.binaryInsert = binaryInsert2;
  }
});
...
__export(main_exports, {
  default: () => FileIndicatorsPlugin
});
module.exports = __toCommonJS(main_exports);
var import_obsidian5 = require("obsidian");
var import_binary_insert = __toESM(require_dist());

// src/indicators/IndicatorModal.ts
var import_obsidian3 = require("obsidian");

// src/shapes/defaults.ts
var defaultShapes = [
  {
    id: "CIRCLE",
    name: "Circle",
...
var getShapeSvg = (shapeId, shapes) => getShapeSvgs(shapes)[shapeId];

// src/shapes/ShapeModal.ts
var import_obsidian = require("obsidian");
var ShapeModal = class extends import_obsidian.Modal {
  constructor(plugin, onSubmit = () => null) {
    super(plugin.app);
    this.plugin = plugin;
  ...
  onOpen() {
    const id = this.plugin.settings.shapes.length > 0 ? this.plugin.settings.shapes.reduce(
      (prev, current) => prev && prev.id > current.id ? prev : current
    ).id + 1 : 1;
    const shape = {
      id,
    ...
    const setting = new import_obsidian.Setting(this.contentEl);
    setting.setClass("fi-list-item");
    setting.addText((text) => {
      text.setPlaceholder("Shape name").setValue(shape.name).onChange((value) => shape.name = value);
    });
    setting.addText((text) => {
      text.setPlaceholder("SVG").setValue(shape.svg).onChange((value) => shape.svg = value);
    });
    const emptyError = new import_obsidian.Setting(this.contentEl);
    emptyError.setDesc("Please set a name and svg.");
    emptyError.settingEl.addClass("fi-modal-error");
    const duplicateError = new import_obsidian.Setting(this.contentEl);
    duplicateError.setDesc("A shape with this name already exists.");
    duplicateError.settingEl.addClass("fi-modal-error");
    const buttonRow = new import_obsidian.Setting(this.contentEl);
    buttonRow.setClass("modal-button-container");
    buttonRow.setClass("fi-modal-buttons");
    buttonRow.addButton((button) => {
      button.setButtonText("Add").setClass("mod-cta").onClick(async () => {
...
// src/suggest/AbstractFileSuggest.ts
var AbstractFileSuggest = class extends TextInputSuggest {
...
// src/indicators/IndicatorModal.ts
var IndicatorModal = class extends import_obsidian3.Modal {
  constructor(plugin, indicator, action = "Add" /* ADD */, onSubmit = () => null) {
    super(plugin.app);
    this.plugin = plugin;
  ...
  onOpen() {
    ...
    setting.setClass("fi-list-item");
    setting.addColorPicker((colorpicker) => colorpicker.setValue(indicator.color).onChange((value) => indicator.color = value.toString()));
    ...
    setting.addDropdown((dropdown) => dropdown.addOptions(getShapeNames(this.plugin.settings.shapes)).setValue(indicator.shape).onChange((value) => indicator.shape = value));
    const emptyError = new import_obsidian3.Setting(this.contentEl);
    emptyError.setDesc("Please set a path.");
    emptyError.settingEl.addClass("fi-modal-error");
    const duplicateError = new import_obsidian3.Setting(this.contentEl);
    duplicateError.setDesc("Path already has an indicator.");
    duplicateError.settingEl.addClass("fi-modal-error");
    const buttonRow = new import_obsidian3.Setting(this.contentEl);
    buttonRow.setClass("modal-button-container");
    buttonRow.setClass("fi-modal-buttons");
    buttonRow.addButton((button) => {
      button.setButtonText(this.action == "Edit" /* EDIT */ ? "Save" : "Add").setClass("mod-cta").onClick(async () => {
        emptyError.settingEl.removeClass("fi-modal-error--active");
        duplicateError.settingEl.removeClass("fi-modal-error--active");
...
var FileIndicatorsSettingTab = class extends import_obsidian4.PluginSettingTab {
  ...
  display() {
    this.containerEl.empty();
    new import_obsidian4.Setting(this.containerEl).setName("Default color").addColorPicker((colorpicker) => colorpicker.setValue(this.plugin.settings.defaultColor).onChange(async (value) => {
      this.plugin.settings.defaultColor = value.toString();
      await this.plugin.saveSettings();
    }));
    this.defaultShapeEL = new import_obsidian4.Setting(this.containerEl);
    this.loadDefaultShape();
    this.shapeListEl.addClass("fi-list");
    this.shapeListEl.addClass("shape-list");
    new import_obsidian4.Setting(this.containerEl).setName("Custom shapes").addButton((button) => {
      button.setButtonText("Add new shape").onClick(() => {
        new ShapeModal(
          this.plugin,
          () => {
    ...
    new import_obsidian4.Setting(this.containerEl).setName("Indicators").addButton((button) => {
      button.setButtonText("Add new indicator").onClick(() => {
        const indicator = {
          dataPath: "",
  ...
  loadDefaultShape() {
    this.defaultShapeEL.clear().setName("Default shape").addDropdown((dropdown) => dropdown.addOptions(getShapeNames(this.plugin.settings.shapes)).setValue(this.plugin.settings.defaultShape.toString()).onChange(async (value) => {
  ...
  addIndicatorListItem(indicator, index = void 0) {
    const setting = new import_obsidian4.Setting(this.indicatorListEl);
    setting.setClass("fi-list-item");
    setting.addColorPicker((colorpicker) => colorpicker.setValue(indicator.color).onChange(async (value) => {
    ...
    setting.addText((text) => {
      text.setPlaceholder("Data path").setValue(indicator.dataPath).setDisabled(true);
    });
    setting.addDropdown((dropdown) => dropdown.addOptions(getShapeNames(this.plugin.settings.shapes)).setValue(indicator.shape).onChange(async (value) => {
      this.plugin.removeIndicatorCSS(indicator);
      if (index !== void 0) {
    ...
    setting.addExtraButton((button) => {
      ...
      }).setTooltip("Delete indicator").setIcon("trash-2");
    });
  }
  loadShapeList() {
  ...
  addShapeListItem(shape) {
    const setting = new import_obsidian4.Setting(this.shapeListEl);
    setting.setClass("fi-list-item");
    const id = "file-indicators-" + shape.id;
    (0, import_obsidian4.addIcon)(id, shape.svg);
    setting.addExtraButton((button) => {
      button.setIcon(id).setDisabled(true);
    });
    setting.addText((text) => {
      text.setPlaceholder("Shape name").setValue(shape.name).setDisabled(true);
    });
    setting.addText((text) => {
      text.setPlaceholder("SVG").setValue(shape.svg).setDisabled(true);
    });
    setting.addExtraButton((button) => {
      button.onClick(async () => {
        const conflictingIndicators = this.plugin.settings.indicators.filter((indicator) => indicator.shape == shape.id);
      ...
      }).setTooltip("Delete shape").setIcon("trash-2");
    });
  }
};

// src/main.ts
var FileIndicatorsPlugin = class extends import_obsidian5.Plugin {
  async onload() {
    ...
    this.registerEvent(this.app.workspace.on("file-menu", (menu, file) => {
      const indicator = this.settings.indicators.find((indicator2) => indicator2.dataPath == file.path);
      if (!indicator) {
        menu.addItem((item) => item.setTitle("Add indicator").setIcon("plus").onClick(() => {
          const indicator2 = {
            dataPath: file.path,
            color: this.settings.defaultColor,
            shape: this.settings.defaultShape
          };
          new IndicatorModal(this, indicator2).open();
        }));
      } else {
        menu.addItem((item) => item.setTitle("Edit indicator").setIcon("pencil").onClick(() => new IndicatorModal(this, indicator, "Edit" /* EDIT */).open()));
        menu.addItem((item) => item.setTitle("Remove indicator").setIcon("minus").onClick(async () => await this.removeIndicator(indicator)));
      }
    }));
    this.registerEvent(this.app.vault.on("rename", async (file, oldPath) => {
      const indicator = this.settings.indicators.find((indicator2) => indicator2.dataPath == oldPath);
      if (indicator !== void 0) {
    ...
    this.addCommand({
      ...
      checkCallback: (checking) => {
        ...
        if (file) {
          ...
          if (indicator) {
            if (!checking) {
              new IndicatorModal(this, indicator, "Edit" /* EDIT */).open();
            }
            return true;
          }
        }
  ...
  async addIndicator(indicator) {
    ...
    (0, import_binary_insert.binaryInsert)(this.settings.indicators, indicator, (a, b) => a.dataPath.localeCompare(b.dataPath));
  ...
  async addShape(shape) {
    (0, import_binary_insert.binaryInsert)(this.settings.shapes, shape, (a, b) => a.name.localeCompare(b.name));
    await this.saveSettings();
  }
